...
class TestBuildPresentation:
    ...
    def test_cue_group_references_all_cues_in_order(self, tmp_path):
        f = tmp_path / "s.tif"
        f.write_bytes(make_tiff())
        slides = [Slide(media_path=str(f)), Slide(media_path=None)]
        pres = build_presentation("Test", slides)
        group_uuids = [ci.string for ci in pres.cue_groups[0].cue_identifiers]
        assert group_uuids == self._cue_uuids(pres)

    def test_presentation_name_set(self):
        pres = build_presentation("My Sermon", [Slide(media_path=None)])
        assert pres.name == "My Sermon"

    def test_all_cue_uuids_unique(self):
        slides = [Slide(media_path=None) for _ in range(5)]
        pres = build_presentation("T", slides)
        uuids = self._cue_uuids(pres)
        assert len(set(uuids)) == len(uuids)

    def test_background_is_white(self):
        pres = build_presentation("T", [Slide(media_path=None)])
        c = pres.background.color
        assert c.red == 1.0 and c.green == 1.0 and c.blue == 1.0 and c.alpha == 1.0

    def test_cues_are_enabled(self, tmp_path):
        f = tmp_path / "s.tif"
        f.write_bytes(make_tiff())
        pres = build_presentation("T", [Slide(media_path=str(f))])
        assert pres.cues[0].isEnabled is True
...
class TestWriteBundle:
    def test_bundle_is_valid_zip(self, tmp_path, image_only_dir):
        ...
        write_bundle(b"\x00", "Pres", slides, bundle)
...
class TestCollectSlides:
    ...
    def test_returns_slide_objects(self, tmp_path):
        d = tmp_path / "in"
        d.mkdir()
        (d / "slide.tif").write_bytes(make_tiff())
        slides = collect_slides(str(d))
    ...
    def test_missing_directory_exits(self, tmp_path):
        with pytest.raises(SystemExit):
            collect_slides(str(tmp_path / "nonexistent"))

    def test_empty_directory_exits(self, tmp_path):
        d = tmp_path / "empty"
        d.mkdir()
        with pytest.raises(SystemExit):
            collect_slides(str(d))
...
class TestEncode:
    def test_creates_probundle_file(self, tmp_path, image_only_dir):
        ...
        encode(out, str(image_only_dir), "Test")
    ...
    def test_output_is_parseable_protobuf(self, tmp_path, image_only_dir):
        ...
        out = str(tmp_path / "out.probundle")
        encode(out, str(image_only_dir), "MyPres")
    ...
    def test_cue_count_matches_input_file_count(self, tmp_path, image_only_dir):
        from pco_types import presentation_pb2 as pp7
        out = str(tmp_path / "out.probundle")
        encode(out, str(image_only_dir), "MyPres")
        with zipfile.ZipFile(out) as zf:
        ...
        pres = pp7.Presentation()
        pres.ParseFromString(pro_bytes)
        assert len(pres.cues) == len(list(image_only_dir.iterdir()))
